...
@property (assign,nonatomic) CGFloat leafEdge;
...
{
    interactionLocked = NO;
    if ([delegate respondsToSelector:@selector(leavesView:didClickPageAtIndex:)]) {
        [delegate leavesView:self didClickPageAtIndex:currentPageIndex];
    }
}
...
- (BOOL)touchLink:(CGPoint)touchPoint
{
    if (currentPageIndex == 0) {
        return CGRectContainsPoint(MUSIC_DEMO_FRAME, touchPoint);  
    } 
    if (currentPageIndex == 1) {
        return CGRectContainsPoint(VIDEO_DEMO_FRAME, touchPoint);  
    }
    return NO;
}
...
- (void)touchesBegan:(NSSet *)touches withEvent:(UIEvent *)event {
    ...
    if ([self touchLink:touchBeganPoint]) {
        if (delegate && [delegate respondsToSelector:@selector(leavesView:didClickLink:atPageIndex:)]) {
            [delegate leavesView:self didClickLink:nil atPageIndex:currentPageIndex];
        }
        return;
    }
    ...
    if (delegate && [delegate respondsToSelector:
                     @selector(leavesView:didBeginTouchPageAtIndex:)]) 
    {
        [delegate leavesView:self didBeginTouchPageAtIndex:currentPageIndex];
    }
